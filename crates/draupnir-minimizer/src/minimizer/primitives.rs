...
mod tests {
    ...
    fn filter_lines_regex_combines_keep_and_strip() {
        ...
        assert_eq!(
            filter_lines_regex(input, Some(&strip), Some(&keep)),
            "task ok\n"
        );
        // Strip only: absent keep set imposes no constraint.
        assert_eq!(
            filter_lines_regex(input, Some(&strip), None),
            "task ok\nunrelated\n"
        );
        // Keep only: absent strip set imposes no constraint.
        assert_eq!(
            filter_lines_regex(input, None, Some(&keep)),
            "task ok\ntask noise\n"
        );
    ...
    fn test_horizontal_rule_requires_non_space() {
        ...
        assert!(
            !is_horizontal_rule("   "),
            "whitespace-only must not be a rule"
        );
        assert!(
            !is_horizontal_rule("  "),
            "short whitespace must not be a rule"
        );
    }
}
