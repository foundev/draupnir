    ...
    while let Some(token) = tokens.get(index) {
        match token.as_str() {
            ...
            | "--close-from" | "-T" | "--command-timeout" => {
                index = skip_option_value(tokens, index)?
            }
...
mod tests {
    ...
    fn skips_cargo_cwd_and_toolchain_globals() {
        ...
        assert_eq!(
            command.as_ref().map(|value| value.program.as_str()),
            Some("cargo")
        );
        assert_eq!(
            command
    ...
    fn detects_gradle_and_maven_wrapper_scripts() {
        ...
        let command =
            detect("gradlew.bat assembleDebug").expect("gradlew.bat normalizes to gradlew");
        assert_eq!(command.program, "gradlew");
...
fn npx_workspace_value_is_skipped_in_subcommand_detection() {
    ...
    assert_eq!(
        ...
        Some("jest"),
        "jest@latest must normalise to jest"
    );
