...
pub fn supports(subcommand: Option<&str>) -> bool {
    matches!(
        subcommand,
        Some("mr" | "issue" | "ci" | "pipeline" | "release")
    )
}
...
pub fn filter(ctx: &MinimizerCtx<'_>, input: &str, exit_code: i32) -> MinimizerOutput {
    ...
    let text = match ctx.subcommand {
        ...
        Some("release")
            if primitives::command_has_ordered_tokens(ctx.command, "release", "list") =>
        {
        ...
        Some("release")
            if primitives::command_has_ordered_tokens(ctx.command, "release", "view") =>
        {
...
fn filter_release_view(input: &str) -> String {
    ...
    for line in input.lines() {
        ...
        if trimmed.starts_with("Image:")
            && (trimmed.contains('\u{2192}') || trimmed.contains(" -> "))
        {
            continue;
        }
...
fn filter_markdown_body_view(input: &str) -> String {
    ...
    for line in input.lines() {
        ...
        if primitives::is_markdown_badge_or_image(trimmed)
            || primitives::is_horizontal_rule(trimmed)
        {
            continue;
        }
        if trimmed.is_empty() {
