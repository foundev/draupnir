...
fn preserves_machine_readable_output(ctx: &MinimizerCtx<'_>) -> bool {
    ...
    if ctx.program == "eslint" {
        ...
        for (i, t) in tokens.iter().enumerate() {
            if (*t == "-f" || *t == "--format")
                && tokens.get(i + 1).is_some_and(|v| *v != "stylish")
            {
                return true;
            }
...
fn is_lint_noise(program: &str, line: &str, exit_code: i32) -> bool {
    ...
    lower.starts_with("checked ")
        ...
        || matches!(program, "pyright" | "basedpyright")
            && lower.starts_with("0 errors, 0 warnings")
...
mod tests {
    ...
    fn pyright_outputjson_passes_through_untouched() {
        ...
        for command in [
            ...
            "basedpyright --outputjson=true src",
        ] {
    ...
    fn supports_common_lint_subcommands_for_future_dispatch() {
        for subcommand in ["check", "lint", "run", "format", "typecheck"] {
            assert!(
                supports(Some(subcommand)),
                "{subcommand} should be supported"
            );
        }
    }
    ...
    fn basedpyright_banner_and_progress_noise_is_stripped() {
        ...
        assert!(
            !out.contains("basedpyright 1.22.0"),
            "version banner must be stripped: {out}"
        );
        assert!(
            !out.contains("Searching for source files"),
            "progress must be stripped: {out}"
        );
        assert!(
            !out.contains("Found 42 source files"),
            "progress must be stripped: {out}"
        );
    ...
    fn pyright_banner_strips_are_scoped_off_other_linters() {
        ...
        assert!(!is_lint_noise(
            "oxlint",
            "Found 3 source files referenced",
            1
        ));
    ...
    fn tsc_eslint_biome_oxlint_are_program_claimed() {
        ...
        for program in ["tsc", "eslint", "biome", "oxlint"] {
            ...
            assert!(
                supports_program(program, None),
                "{program} bare invocation must be claimed"
            );
        }
    }
    ...
    fn tsc_pretty_strips_code_frames_and_groups_by_file() {
        ...
        assert!(
            !out.contains("foo = 1;"),
            "code-frame body must be stripped: {out}"
        );
        assert!(
            !out.contains("const x: number"),
            "code-frame body must be stripped: {out}"
        );
    ...
    fn eslint_stylish_groups_rows_under_headers_with_top_rules() {
        ...
        assert!(
            out.contains("Parsing error: Unexpected token"),
            "got: {out}"
        );
        ...
        assert!(
            !out.contains("potentially fixable"),
            "fixable hint must be stripped: {out}"
        );
    }
    ...
    fn eslint_stylish_single_file_keeps_summary() {
        ...
        assert!(
            out.contains("✖ 1 problem (1 error, 0 warnings)"),
            "got: {out}"
        );
    }
    ...
    fn biome_ci_keeps_diagnostics_strips_code_frames() {
        ...
        assert!(
            out.contains('\u{00d7}'),
            "× diagnostic rows must survive: {out}"
        );
        ...
        assert!(
            ...
            "checked chatter must be stripped: {out}"
        );
        assert!(
            !out.contains('^'),
            "caret underline must be stripped: {out}"
        );
    }
    ...
    fn oxlint_keeps_rule_and_location_strips_frames() {
        ...
        assert!(
            !out.contains('\u{2570}'),
            "box closer must be stripped: {out}"
        );
        assert!(
            !out.contains("Finished in"),
            "Finished chatter must be stripped: {out}"
        );
        assert!(
            !out.contains("Found 2 warnings"),
            "Found chatter must be stripped: {out}"
        );
    }
    ...
    fn shared_python_ruby_paths_unaffected_by_js_strips() {
        ...
        assert!(
            out.contains("attr-defined") || out.contains("b.py"),
            "got: {out}"
        );
    }
    ...
    fn js_lint_numeric_summary_line_is_not_gutter_stripped() {
        ...
        assert!(
            out.contains("\u{2716} 9 problems (9 errors, 0 warnings)"),
            "got: {out}"
        );
        ...
        assert!(
            !is_gutter_bar_line("10 errors found"),
            "bare numeric is not a bar gutter"
        );
        assert!(
            is_gutter_bar_line("3 \u{2502} interface Props {"),
            "biome bar gutter strips"
        );
        assert!(
            is_gutter_bar_line("12 \u{2502} items.forEach(...)"),
            "oxlint bar gutter strips"
        );
    }
    ...
    fn eslint_stylish_still_condensed() {
        ...
        assert!(
            out.changed,
            "eslint --format stylish must go through the condenser"
        );
    }
    ...
    fn eslint_no_format_flag_still_condensed() {
        ...
        assert!(
            out.changed,
            "eslint without -f must go through the condenser"
        );
    }
    ...
    fn eslint_hyphenless_rules_counted_under_top_rules_and_unglued() {
        ...
        let out = condense_lint_output("eslint", input, 1);
        assert!(
            out.contains("Top rules:"),
            "hyphenless rules must produce a Top rules line: {out}"
        );
