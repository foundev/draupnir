...
mod tests {
    ...
    fn pytest_caps_failure_blocks_with_overflow_marker() {
        ...
        for i in 0..12 {
            let _ = writeln!(
                input,
                "FAILED tests/test_many.py::test_case_{i} - assert False"
            );
        }
    ...
    fn pytest_xfail_xpass_summary_lines_survive() {
        ...
        assert!(
            out.contains("XFAIL tests/test_x.py::test_known_bug"),
            "got: {out}"
        );
        assert!(
            out.contains("XPASS tests/test_x.py::test_unexpected_pass"),
            "got: {out}"
        );
        assert!(
            out.contains("pytest: 1 passed, 1 xfailed, 1 xpassed"),
            "got: {out}"
        );
    }
    ...
    fn pytest_collection_errors_do_not_evict_real_failure_tracebacks() {
        ...
        for i in 0..5 {
            let _ = writeln!(
                input,
                "FAILED tests/pay.py::test_critical_{i} - assert 0 == 100"
            );
        }
        input.push_str(
        ...
        for i in 0..5 {
            assert!(
                out.contains(&format!("test_critical_{i}")),
                "lost failure {i}: {out}"
            );
        }
        ...
        assert_eq!(
            assert_lines, 5,
            "all 5 assertion bodies must survive: {out}"
        );
        // 5 < cap, so there is NO failure overflow stealing real tracebacks.
        assert!(
            !out.contains("failures elided"),
            "no failures should be capped: {out}"
        );
        // Collection errors are still surfaced (under their own cap of 10).
        assert!(
            out.contains("ERROR collecting tests/test_imp_0.py"),
            "got: {out}"
        );
        assert!(
            !out.contains("errors elided"),
            "9 errors < cap, no error overflow: {out}"
        );
    ...
    fn pytest_caps_error_blocks_independently_of_failures() {
        ...
        assert!(
            !out.contains("failures elided"),
            "failures must be uncapped here: {out}"
        );
