...
mod tests {
    ...
    fn groups_grep_by_file() {
        ...
        assert!(
            out.text.starts_with("grep: 2 matches in 1 files"),
            "{:?}",
            out.text
        );
    ...
    fn center_truncate_long_line_with_leading_whitespace_centers_in_code() {
        ...
        assert!(
            ...
            "test line must exceed max_chars"
        );
        ...
        assert!(
            out.contains("result"),
            "match region 'result' should be visible: {out}"
        );
    ...
    fn center_truncate_long_line_no_whitespace_centers_in_middle() {
        ...
        assert!(
            out.contains("Module14"),
            "middle modules should be visible: {out}"
        );
        assert!(
            !out.starts_with("use std"),
            "front content should be dropped: {out}"
        );
    }
    ...
    fn df_keeps_overlay_root_drops_inner_overlays() {
        ...
        assert!(
            !out.text.contains("tmpfs"),
            "tmpfs must be dropped; got:\n{}",
            out.text
        );
    }
    ...
    fn aggressive_rust_outline_bails_on_raw_strings() {
        let cfg = aggressive_cfg();
        let ctx = ctx_command("cat", "cat src/foo.rs", &cfg);
        let body = "pub fn shader() -> &'static str {\n    r#\"fn main() { println!(\\\"hi\\\"); }\"#\n}\n";
        let out = filter(&ctx, body, 0);
        assert!(
            !out.changed,
            "raw-string Rust source should fall back to default outline"
        );
    ...
    fn aggressive_cat_with_behavior_flags_preserves_output() {
        ...
        assert!(
            !out.changed,
            "cat flags that alter output must remain verbatim"
        );
        assert_eq!(out.text, body);
    }
    ...
    fn grep_null_data_flag_bypasses_compaction() {
        ...
        for cmd in &["grep -zHn pattern file", "grep --null-data pattern file"] {
            ...
            assert_eq!(
                out.text, input,
                "grep NUL-output flag must passthrough: {cmd}"
            );
        }
    }
    ...
    fn rg_null_flag_bypasses_compaction() {
        ...
        for cmd in &["rg -0 pattern", "rg --null pattern"] {
            ...
            assert_eq!(
                out.text, input,
                "rg NUL-output flag must passthrough: {cmd}"
            );
        }
    }
    ...
    fn parse_ls_long_line_handles_spaced_filename() {
        let entry =
            parse_ls_long_line("drwxr-xr-x  2 user staff  192 Feb  2 21:35 my dir").unwrap();
    ...
    fn compact_ls_with_spaced_filename_and_year_date() {
        ...
        let out = filter(&ctx, &input, 0);
        assert!(
            out.text.contains("my dir/"),
            "spaced dir name missing: {}",
            out.text
        );
        assert!(
            out.text.contains("my file.txt"),
            "spaced file name missing: {}",
            out.text
        );
        assert!(
            out.text.contains("archive.tar"),
            "year-format file missing: {}",
            out.text
        );
    }
    ...
    fn long_df_drops_virtual_fs_rows() {
        ...
        assert!(
            out.text.contains("/dev/disk"),
            "real disk rows must be kept: {}",
            out.text
        );
        assert!(
            out.text.contains("…"),
            "summary ellipsis expected: {}",
            out.text
        );
    }
