...
fn filter_kubectl(ctx: &MinimizerCtx<'_>, input: &str, exit_code: i32) -> String {
    ...
    match ctx.subcommand {
        ...
        Some(
            "apply" | "delete" | "rollout" | "scale" | "create" | "wait" | "label" | "annotate",
...
fn compute_pod_container_stats(status: &Value) -> (usize, usize, i32) {
    let Some(container_statuses) = status.get("containerStatuses").and_then(|v| v.as_array())
    else {
        return (0, 0, 0);
    };
...
mod tests {
    ...
    fn dedups_compose_service_prefixed_log_messages() {
        ...
        assert!(
            ...
            "distinct service must be preserved: {out}"
        );
        assert!(
        ...
        assert!(
            out.contains("api | ready"),
            "distinct service must be preserved: {out}"
        );
        assert!(
            out.contains("worker | busy"),
            "distinct service must be preserved: {out}"
        );
    }
    ...
    fn docker_compose_logs_uses_log_filter() {
        ...
        assert!(
            out.contains("api-1  | ready"),
            "distinct service line must be preserved: {out}"
        );
        assert!(
            out.contains("api-2  | ready"),
            "distinct service line must be preserved: {out}"
    ...
    fn compose_exec_with_service_named_logs_is_not_log_command() {
        ...
        for cmd in &[
            ...
            assert!(
                !is_log_command(&ctx),
                "`{cmd}` must not be classified as a log command"
            );
        }
    }
    ...
    fn compose_up_with_service_named_ps_is_not_table_command() {
        ...
        for cmd in &[
            ...
            assert!(
                ...
                "`{cmd}` must not be classified as a table command"
            );
        }
    }
    ...
    fn docker_ps_quiet_preserves_id_listing_verbatim() {
        ...
        assert_eq!(
            out, input,
            "docker ps -q output is machine-readable and must not be compacted"
        );
    }
    ...
    fn docker_images_format_table_still_compacts() {
        ...
        assert!(
            out.contains("25 rows"),
            "docker --format table output should still compact: {out}"
        );
        assert!(
    ...
    fn docker_compose_ps_format_without_table_preserves_template_output_verbatim() {
        ...
        assert_eq!(
            out, input,
            "docker compose ps formatted output must not be compacted"
        );
    }
    ...
    "kind": "List"
        ...
        assert!(out.contains(
            "prod/lb-svc\tLoadBalancer\t10.0.0.20\t203.0.113.1\t443/8443:30001->443/TCP"
        ));
    ...
    fn helm_template_keeps_manifest_yaml_opaque() {
        ...
        assert_eq!(
            out, input,
            "helm template output must be preserved verbatim"
        );
    }
    ...
    fn kubectl_get_ojson_attached_preserves_json() {
        ...
        assert_eq!(
            out, input,
            "-ojson must passthrough verbatim, not be compacted to a table"
        );
    }
    ...
    fn kubectl_get_oname_attached_skips_table_compaction() {
        ...
        assert!(
            !out.contains("rows"),
            "-oname output must not be table-compacted, got: {out}"
        );
    }
    ...
    fn kubectl_get_ojsonpath_attached_skips_table_compaction() {
        ...
        assert!(
            !out.contains("rows"),
            "-ojsonpath output must not be table-compacted, got: {out}"
        );
    }
    ...
    fn kubectl_get_owide_attached_still_compacts_table() {
        ...
        assert!(
            out.contains("rows"),
            "-owide is a table format and must be compacted, got: {out}"
        );
    }
    ...
    fn dedups_non_consecutive_log_lines_globally() {
        ...
        let out = filter_docker_logs(input);
        assert!(
            out.contains("ready (×3)"),
            "global dedup should count all occurrences: {out}"
        );
        assert!(
            out.contains("starting (×2)"),
            "global dedup should count all occurrences: {out}"
        );
        assert!(
            out.contains("done"),
            "global dedup should preserve unique lines: {out}"
        );
    }
    ...
    fn helm_list_strips_glog_warning_lines() {
        ...
        assert!(
            !out.contains("W0115"),
            "glog W-prefixed lines should be stripped: {out}"
        );
        assert!(
            out.contains("NAME: my-release"),
            "release info should be preserved: {out}"
        );
        assert!(
            out.contains("STATUS: deployed"),
            "release info should be preserved: {out}"
        );
    }
    ...
    fn docker_logs_dedups_empty_compose_messages_per_service() {
        ...
        assert!(
            out.contains("api |  (×3)"),
            "same-service empty messages should dedup: {out}"
        );
        assert!(
            out.contains("worker |  (×2)"),
            "same-service empty messages should dedup: {out}"
        );
    }
    ...
    fn helm_list_preserves_release_names_starting_with_w_digit() {
        ...
        assert!(
            out.contains("my-app"),
            "other releases must be preserved: {out}"
        );
    }
    ...
    fn docker_compose_p_project_name_routes_logs() {
        ...
        assert!(
            is_log_command(&ctx),
            "docker compose -p myproj logs must be a log command"
        );
    }
