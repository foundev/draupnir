...
mod tests {
    ...
    fn detect_phase_ignores_pl_module_value() {
        ...
        assert_eq!(
            detect_phase("mvn install -pl :sub1,:sub2"),
            MvnPhase::Package
        );
    }
    #[test]
    fn detect_phase_skips_value_of_value_taking_options() {
        ...
        assert_eq!(
            detect_phase("mvn install --projects test"),
            MvnPhase::Package
        );
    ...
    fn filter_surefire_pass_output_compact() {
        ...
        assert!(
            savings >= 50.0,
            "pass-fixture savings >=50%, got {savings:.1}%"
        );
    }
    ...
    fn surefire_matches_warning_skipped_close_line() {
        ...
        assert!(
            ...
            "[WARNING] close-line matched; got:\n{o}"
        );
    }
    ...
    fn surefire_rearm_disarms_at_results_boundary() {
        ...
        assert!(
            ...
            "Results boundary disarms re-arm; got:\n{o}"
        );
        assert!(
    ...
    fn surefire_tolerates_double_blank_between_failure_blocks() {
        ...
        assert!(
            ...
            "second block re-enters trail; got:\n{o}"
        );
        assert!(
            !o.contains("\n\n\n"),
    ...
    fn savings_mvn_test_multifail_slice() {
        ...
        assert!(
            savings >= 30.0,
            "multifail slice >=30% savings, got {savings:.1}%"
        );
    }
    ...
    fn footer_guard_french_passthrough() {
        ...
        assert!(
            ...
            "footer-guard passes through non-English; got:\n{o}"
        );
        assert_eq!(
            o.lines().count(),
            i.lines().count(),
    ...
    fn surefire_caps_failing_blocks_emits_tail() {
        ...
        assert!(
            o.contains("[…2 failing test classes elided…]"),
            "tail emitted; got:\n{o}"
        );
    }
    ...
    fn surefire_cap_zero_emits_summary_only() {
        ...
        assert!(
            o.contains("[…5 failing test classes elided…]"),
            "tail counts all 5; got:\n{o}"
        );
    }
    ...
    fn failures_summary_block_is_capped() {
        ...
        for n in 1..=3 {
            assert!(
                o.contains(&format!("ClassA.test{n}:25")),
                "entry {n} kept; got:\n{o}"
            );
        }
        for n in 4..=5 {
    ...
    fn reactor_summary_kept_on_multi_module_fail() {
        ...
        assert!(
            savings >= 30.0,
            "reactor-fail slice savings >=30%; got {savings:.1}%"
        );
    }
    ...
    fn filter_compile_error_compact() {
        ...
        assert!(
            savings >= 30.0,
            "compile-error fixture small; >=30% savings, got {savings:.1}%"
        );
    }
    ...
    fn filter_package_install_compact() {
        ...
        assert!(
            savings >= 50.0,
            "install-slice savings >=50%, got {savings:.1}%"
        );
    }
    ...
    fn savings_mvn_quiet_fail() {
        ...
        assert!(
            savings >= 50.0,
            "mvn -q fail >=50% savings, got {savings:.1}%"
        );
    }
    ...
    fn gradle_build_strips_tasks_keeps_status() {
        ...
        assert!(
            out.text.contains("BUILD SUCCESSFUL in 8s"),
            "status kept; got:\n{}",
            out.text
        );
        assert!(
    ...
    fn dispatch_clean_install_routes_to_package() {
        ...
        assert!(
            out.text.contains("Building jar:"),
            "package keepers applied; got:\n{}",
            out.text
        );
    }
    ...
    fn passthrough_keeps_build_failure_drops_download_noise() {
        ...
        assert!(
            !o.contains("Downloading from central"),
            "download noise dropped; got:\n{o}"
        );
        assert!(
            !o.contains("[INFO] -----"),
            "banner dashes dropped; got:\n{o}"
        );
    }
    ...
    fn gradle_detect_connected_wins_over_test() {
        ...
        assert_eq!(
            detect_task("gradlew connectedDebugAndroidTest"),
            GradleTask::ConnectedTest
        );
    }
    #[test]
    ...
    fn gradle_detect_module_prefixed_task() {
        assert_eq!(
            detect_task("gradlew :app:testDebugUnitTest"),
            GradleTask::Test
        );
    }
    #[test]
    ...
    fn gradle_detect_flag_value_does_not_trigger_test() {
        ...
        assert_eq!(
            detect_task("gradlew assembleRelease -Pflavor=testRelease"),
            GradleTask::Build
        );
    }
    #[test]
    fn gradle_detect_multi_task_uses_last() {
        ...
        assert_eq!(
            detect_task("gradlew clean assembleDebug"),
            GradleTask::Build
        );
    }
    #[test]
    ...
    fn gradle_detect_dependencies() {
        assert_eq!(
            detect_task("gradlew dependencies"),
            GradleTask::Dependencies
        );
    }
    #[test]
    fn gradle_detect_dependencies_with_module() {
        assert_eq!(
            detect_task("gradlew :app:dependencies"),
            GradleTask::Dependencies
        );
    }
    #[test]
    fn gradle_detect_boot_run_routes_to_spring() {
        ...
        assert_eq!(
            detect_task("gradlew :app:bootRun"),
            GradleTask::SpringBootRun
        );
    }
    #[test]
    fn detect_task_ignores_tests_filter_value() {
        ...
        assert_eq!(
            detect_task("./gradlew test --tests FooSpec"),
            GradleTask::Test
        );
        assert_eq!(
            detect_task("./gradlew test --tests com.example.Foo"),
            GradleTask::Test
        );
    }
    #[test]
    fn detect_task_skips_value_of_value_taking_options() {
        ...
        assert_eq!(
            detect_task("gradle --project-dir test build"),
            GradleTask::Build
        );
    }
    ...
    fn gradle_build_failure_preserves_errors_strips_try() {
        ...
        assert!(
            !o.contains("Run with --stacktrace"),
            "try section stripped; got:\n{o}"
        );
        assert!(
            !o.contains("Get more help at"),
            "help link stripped; got:\n{o}"
        );
    }
    ...
    fn gradle_build_filter_never_empty_on_success() {
        ...
        assert!(
            !o.trim().is_empty(),
            "must not be empty on success; got:\n{o}"
        );
    ...
    fn gradle_build_scan_url_preserved() {
        ...
        let o = filter_gradle_build(input);
        assert!(
            o.contains("gradle.com/s/"),
            "build scan URL kept; got:\n{o}"
        );
    }
    ...
    fn gradle_build_keeps_compiler_warnings() {
        ...
        assert!(
            o.contains("warning: [options]"),
            "javac warnings kept; got:\n{o}"
        );
        assert!(
            o.contains("Warning: Gradle"),
            "Gradle warnings kept; got:\n{o}"
        );
        assert!(o.contains("BUILD SUCCESSFUL"));
    ...
    fn gradle_build_strips_configure_and_dokka_noise() {
        ...
        assert!(
            o.contains("Execution failed"),
            "Execution failed kept; got:\n{o}"
        );
        assert!(
            o.contains("Lint found 8 error"),
            "lint summary kept; got:\n{o}"
        );
        assert!(
            o.contains("Error: Field requires"),
            "lint error kept; got:\n{o}"
        );
        assert!(
            !o.contains("Configure project"),
            "configure stripped; got:\n{o}"
        );
        assert!(!o.contains("dokka"), "dokka warnings stripped; got:\n{o}");
        assert!(
            !o.contains("android-junit5"),
            "plugin warnings stripped; got:\n{o}"
        );
        assert!(!o.contains("> Task :"), "task lines stripped; got:\n{o}");
        assert!(!o.contains("Incubating"), "incubating stripped; got:\n{o}");
        assert!(
            !o.contains("Deprecated Gradle"),
            "deprecated stripped; got:\n{o}"
        );
        assert!(
            !o.contains("Run with --stacktrace"),
            "try section stripped; got:\n{o}"
        ...
        assert!(
            savings >= 60.0,
            "mixed-output savings >=60%, got {savings:.1}%"
        );
    }
    ...
    fn gradle_unit_test_failures_preserved_passes_stripped() {
        ...
        assert!(
            o.contains("AssertionError"),
            "exception class kept; got:\n{o}"
        );
        assert!(o.contains("FooTest.testBar"), "user frame kept; got:\n{o}");
        assert!(
            !o.contains("org.junit.Assert.fail"),
            "framework frames skipped; got:\n{o}"
        );
        assert!(!o.contains("PASSED"), "PASSED stripped; got:\n{o}");
        assert!(
            o.contains("10 tests completed, 1 failed"),
            "summary kept; got:\n{o}"
        );
    ...
    fn gradle_unit_test_skips_framework_frames() {
        ...
        let o = filter_gradle_test(input);
        assert!(
            o.contains("com.example.CalcTest.testAdd"),
            "user frame kept; got:\n{o}"
        );
        assert!(
            !o.contains("org.junit.Assert"),
            "JUnit frames skipped; got:\n{o}"
        );
        assert!(
            !o.contains("java.lang.reflect"),
            "reflection frames skipped; got:\n{o}"
        );
    }
    ...
    fn gradle_unit_test_report_path_preserved() {
        ...
        let o = filter_gradle_test(input);
        assert!(
            o.contains("See the report at"),
            "report pointer kept; got:\n{o}"
        );
        assert!(o.contains("BUILD FAILED"), "status kept; got:\n{o}");
    }
    ...
    fn gradle_connected_strips_device_noise() {
        ...
        assert!(
            !o.contains("INSTRUMENTATION_STATUS:"),
            "instrumentation stripped; got:\n{o}"
        );
        assert!(
            !o.contains("Starting 3 tests"),
            "starting line stripped; got:\n{o}"
        );
    }
    ...
    fn gradle_connected_no_device_error() {
        ...
        let o = filter_gradle_connected(input);
        assert!(
            o.contains("No connected devices"),
            "actionable error shown; got:\n{o}"
        );
    }
    ...
    fn gradle_lint_preserves_violations() {
        ...
        assert!(
            o.contains("StringFormatInvalid"),
            "violation kept; got:\n{o}"
        );
        assert!(
            o.contains("0 errors, 4 warnings"),
            "summary kept; got:\n{o}"
        );
        assert!(
            !o.contains("Wrote HTML report"),
            "report path stripped; got:\n{o}"
        );
    }
    ...
    fn gradle_lint_preserves_warnings_and_context() {
        ...
        assert!(
            o.contains("return \"Hello World\""),
            "code snippet kept; got:\n{o}"
        );
    ...
    fn gradle_dependencies_extracts_top_level() {
        ...
        assert!(
            o.contains("implementation (5):"),
            "config with count; got:\n{o}"
        );
        assert!(
            o.contains("testImplementation (2):"),
            "test config; got:\n{o}"
        );
        assert!(o.contains("kotlin-stdlib"), "top-level dep; got:\n{o}");
        assert!(
            !o.contains("cursoradapter"),
            "transitive deps stripped; got:\n{o}"
        );
        let savings = savings_pct(input, &o);
    ...
    fn gradle_build_empty_line_preserved() {
        ...
        assert!(
            keep_gradle_build_line("   "),
            "whitespace-only line passes through"
        );
    }
    ...
    fn gradle_other_task_light_strip() {
        ...
        assert!(
            !o.contains("Starting a Gradle Daemon"),
            "daemon stripped; got:\n{o}"
        );
        assert!(!o.contains("> Task :"), "task progress stripped; got:\n{o}");
    }
    ...
    fn gradle_def_strips_up_to_date_and_keeps_result() {
        ...
        let o = filter_gradle_build(input);
        assert!(
            o.contains("3 tests completed, 1 failed"),
            "test summary kept; got:\n{o}"
        );
    ...
    fn gradle_def_clean_build_untouched() {
        ...
        let o = filter_gradle_build(input);
        assert!(
            o.contains("BUILD SUCCESSFUL in 8s"),
            "status kept; got:\n{o}"
        );
        assert!(
            o.contains("7 actionable tasks: 7 executed"),
            "actionable kept; got:\n{o}"
        );
    }
    ...
    fn gradle_def_empty_after_stripping() {
        ...
        assert!(
            o.trim().is_empty(),
            "all noise stripped to empty; got:\n{o}"
        );
    }
    ...
    fn gradle_snip_build_success() {
        ...
        let o = filter_gradle_build(input);
        assert!(
            o.contains("BUILD SUCCESSFUL in 2s"),
            "status kept; got:\n{o}"
        );
        assert!(
            o.contains("5 actionable tasks: 5 executed"),
            "actionable kept; got:\n{o}"
        );
    ...
    fn gradle_snip_build_failure_with_exception() {
        ...
        let o = filter_gradle_build(input);
        assert!(
            o.contains("FAILURE: Build failed with an exception."),
            "FAILURE kept; got:\n{o}"
        );
        assert!(
            o.contains("Compilation error: package com.example"),
            "error detail kept; got:\n{o}"
        );
    ...
    fn gradle_snip_download_and_start_noise_removed() {
        ...
        let o = filter_gradle_build(input);
        assert!(
            o.contains("Test result: FAILURE"),
            "test result kept; got:\n{o}"
        );
    ...
    fn gradle_dispatch_test_task_routes_to_test_filter() {
        ...
        assert!(
            out.text.contains("t2 FAILED"),
            "failing test kept; got:\n{}",
            out.text
        );
        assert!(
            !out.text.contains("PASSED"),
            "passing stripped; got:\n{}",
            out.text
        );
    }
    ...
    fn spring_boot_keeps_startup_summary_and_errors() {
        ...
        assert!(
            o.contains("Tomcat started on port 8080"),
            "tomcat line kept; got:\n{o}"
        );
        assert!(
            o.contains("Started MyApp in 3.2 seconds"),
            "started line kept; got:\n{o}"
        );
        assert!(
            !o.contains("Initializing Spring"),
            "INFO chatter stripped; got:\n{o}"
        );
        assert!(
            !o.contains("Bean 'dataSource'"),
            "bean chatter stripped; got:\n{o}"
        );
        assert!(
            !o.contains(":: Spring Boot ::"),
            "banner stripped; got:\n{o}"
        );
    }
    ...
    fn spring_boot_preserves_errors() {
        ...
        let o = filter_spring_boot(input);
        assert!(
            o.contains("ERROR Application run failed"),
            "error kept; got:\n{o}"
        );
        assert!(
            o.contains("Caused by: java.lang.NullPointerException"),
            "cause kept; got:\n{o}"
        );
        assert!(
            !o.contains("Initializing Spring"),
    ...
    fn spring_boot_routes_from_mvn_goal() {
        ...
        assert!(
            out.text.contains("Tomcat started on port 8080"),
            "tomcat kept; got:\n{}",
            out.text
        );
        assert!(
            !out.text.contains("Bean created"),
            "INFO stripped; got:\n{}",
            out.text
        );
    }
    ...
    fn spring_boot_routes_from_gradle_boot_run() {
        ...
        assert!(out.changed, "spring-boot filter transformed output");
        assert!(
            out.text.contains("ERROR Application run failed"),
            "error kept; got:\n{}",
            out.text
        );
        assert!(
            out.text.contains("Caused by:"),
            "cause kept; got:\n{}",
            out.text
        );
        assert!(
    ...
    fn spring_boot_savings() {
        ...
        assert!(
            savings >= 50.0,
            "spring-boot savings >=50%, got {savings:.1}%"
        );
    }
}
