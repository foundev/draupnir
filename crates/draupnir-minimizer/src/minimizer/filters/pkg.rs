...
fn is_poetry_bullet_progress(lower: &str) -> bool {
    ...
    if let Some(rest) = bullet
        && (rest.starts_with("downloading ")
            || rest.starts_with("installing ") && rest.contains('('))
    {
        return true;
    }
...
fn is_uv_progress_noise(lower: &str, keep_install_summary: bool) -> bool {
    if keep_install_summary
        && (lower.starts_with("installed ") || lower.starts_with("uninstalled "))
    {
        return false;
    }
    lower.starts_with("resolved ")
...
mod tests {
    ...
    fn preserves_deprecation_warnings() {
        ...
        let out = strip_package_noise(&ctx, input, 0);
        assert!(
            out.contains("npm warn deprecated left-pad@1.0.0: Please upgrade to left-pad@2.0.0")
        );
    ...
    fn test_pip_list_format_json_passthrough() {
        ...
        assert_eq!(
            out.text, input,
            "pip list --format=json must pass through as JSON"
        );
