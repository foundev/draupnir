...
pub fn filter(ctx: &MinimizerCtx<'_>, input: &str, exit_code: i32) -> MinimizerOutput {
    ...
    let text = match ctx.subcommand {
        ...
        Some(
            "merge" | "rebase" | "checkout" | "switch" | "restore" | "clean" | "reset" | "add",
...
fn is_status_machine_format(command: &str) -> bool {
    command.split_whitespace().any(|part| {
        matches!(
            part,
            "--porcelain" | "--porcelain=v1" | "--porcelain=v2" | "--null"
        ) || part == "-z"
...
fn compact_diff_listing(input: &str, mode: DiffListingMode) -> String {
    ...
    let _ = writeln!(
        out,
        "[…{} files elided…]",
        entries.len() - DIFF_LISTING_LIMIT
    );
    out
}
...
";
    ...
    fn push_output_keeps_deleted_refs() {
        ...
        let out = filter(
            &ctx,
            "To github.com:can1357/oh-my-pi.git\n - [deleted]         old-branch\n",
            0,
        );
        assert!(out.changed);
    ...
    fn show_custom_format_passes_through_inline_and_space_forms() {
        ...
        for command in custom {
            ...
            assert!(
                !out.changed,
                "`{command}` must pass through custom-format show output"
            );
    ...
    fn log_long_subject_is_truncated_with_dropped_marker() {
        ...
        assert!(
            !out.text.contains(&long_subject),
            "full subject must not survive"
        );
    }
...
";
    ...
    fn diff_name_only_failure_keeps_diagnostics() {
        ...
        let input = "fatal: ambiguous argument 'badrev': unknown revision or path not in the working tree.\n";
...
";
    ...
    fn status_detects_merge_all_conflicts_fixed() {
        ...
        assert!(out.changed);
        assert!(
            out.text.starts_with("state: merge (ready to commit)\n"),
            "{:?}",
            out.text
        );
    ...
    fn status_untracked_file_named_like_rebase_progress_not_misread_as_state() {
        ...
        assert!(
            out.text.contains("?? Next command to do.md"),
            "{:?}",
            out.text
        );
    }
    ...
    fn log_failure_keeps_diagnostics() {
        ...
        assert!(
            out.text.contains("fatal:"),
            "error header must survive: {:?}",
            out.text
        );
        assert!(
            out.text.contains("badref"),
            "offending ref must survive: {:?}",
            out.text
        );
        assert!(
            !out.text.contains("commits elided"),
            "must not fabricate commit listing on error"
        );
    }
...
commit abcdef1234567890\nAuthor: A <a@x.com>\nDate: today\n    feat: first\ncommit \
        ...
        assert!(
            !out.text.contains("Author:"),
            "author noise must be stripped"
        );
...
Auto-merging src/lib.rs\nCONFLICT (content): Merge conflict in src/lib.rs\nAutomatic merge failed; \
        ...
        assert!(
            out.text.contains("CONFLICT"),
            "conflict marker must survive: {:?}",
            out.text
        );
        assert!(
            out.text.contains("src/lib.rs"),
            "conflict path must survive: {:?}",
            out.text
        );
        assert!(
            !out.text.contains("ok"),
            "must not emit an ok summary on failure"
        );
    }
...
error: could not apply abc1234... fix: something\nhint: Resolve all conflicts manually, mark them \
        ...
        assert!(
            out.text.contains("src/config.rs"),
            "conflict path must survive: {:?}",
            out.text
        );
    }
    ...
    fn worktree_list_caps_and_abbreviates_home() {
        ...
        assert!(
            ...
            "no absolute $HOME paths should survive: {out:?}"
        );
    ...
    fn worktree_porcelain_passthrough() {
        ...
        assert!(
            !out.changed,
            "porcelain worktree listing must pass through unchanged"
        );
        assert_eq!(
            out.text, input,
            "porcelain worktree listing must pass through"
        );
    }
    ...
    fn worktree_null_passthrough() {
        ...
        assert!(
            !out.changed,
            "-z worktree output must pass through unchanged"
        );
        assert_eq!(out.text, input);
    }
