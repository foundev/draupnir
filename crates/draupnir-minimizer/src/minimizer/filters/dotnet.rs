    ...
    if exit_code == 0 && matches!(label, "dotnet build" | "dotnet restore") {
        ...
        if let Some(i) = last_match {
            ...
            let has_later_issues = lines[i + 2..].iter().any(|line| {
                ...
                let is_count_summary = (lower.ends_with("warning(s)")
...
fn compact_format_json(input: &str) -> Option<String> {
    ...
    for row in rows.iter().take(40) {
        out.push_str(&primitives::truncate_line(
            row,
            primitives::CapClass::Errors.lines(),
        ));
        out.push('\n');
    }
    if rows.len() > 40 {
...
fn collect_format_json_rows(
    ...
    match value {
        serde_json::Value::Object(map) => {
            let path = first_string(map, &["FileName", "FilePath", "Path", "DocumentPath"])
                .or(inherited_path);
