...
fn filter_go_test(input: &str, exit_code: i32) -> String {
    ...
    for line in input.lines() {
        ...
        if let Some(rendered) = render_go_test_json_line(trimmed) {
            ...
            let keep_line =
                keep_next_after_location || should_keep_go_test_line(&rendered, exit_code);
...
"#;
    ...
    fn go_test_success_default_text_counts_no_tests_and_skips() {
        ...
        assert_eq!(
            out.text.trim(),
            "go test: 3 packages ok, 1 no tests, 1 tests skipped"
        );
    }
    ...
    fn go_benchmark_preserved_on_success() {
        ...
        assert!(
            result.contains("BenchmarkFoo"),
            "benchmark stripped: {result}"
        );
    }
...
{"Action":"pass","Elapsed":1.234}"#;
        ...
        assert!(
            !result.contains("packages ok"),
            "benchmark json run must not be collapsed"
        );
        assert!(
            result.contains("BenchmarkFoo"),
            "benchmark lines must survive"
        );
    }
}
