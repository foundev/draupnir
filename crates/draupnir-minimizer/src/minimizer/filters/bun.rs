...
fn bun_wrapper_option_takes_value(token: &str) -> bool {
    matches!(
        token,
        "--filter" | "--cwd" | "--env-file" | "--preload" | "-F" | "-C" | "-r"
    )
}
...
fn is_lint_invocation(program: &str, subcommand: Option<&str>, command: &str) -> bool {
    matches!(
        (program, subcommand),
        ("bun" | "bunx", Some("tsc" | "eslint" | "biome"))
    ) || is_exec_package_subcommand(program, subcommand)
        && command_invoked_word(command).is_some_and(|token| {
...
fn is_js_tool_invocation(program: &str, subcommand: Option<&str>, command: &str) -> bool {
    matches!(
        (program, subcommand),
        ("bun" | "bunx", Some("next" | "prettier" | "prisma"))
    ) || is_exec_package_subcommand(program, subcommand)
...
mod tests {
    ...
    fn supports_bun_package_test_and_tool_subcommands() {
        for subcommand in [
            "install", "add", "run", "test", "build", "tsc", "next", "ctest", "check",
        ] {
            assert!(
                supports("bun", Some(subcommand)),
    ...
    fn bun_check_direct_subcommand_is_supported_and_routes_to_check_filter() {
        ...
        assert!(
            supports("bun", Some("check")),
            "bun check should be supported"
        );
        ...
        assert!(
            ...
            "check filter must preserve error output"
        );
    }
    ...
    fn bun_add_known_tool_package_names_use_package_filter() {
        ...
        for package in ["eslint", "prettier", "jest"] {
            ...
            assert!(
                !out.text.contains("Downloaded"),
                "{package} should strip package download noise"
            );
            assert!(out.text.contains("error: failed"));
        }
    }
    ...
    fn bun_next_build_uses_next_route_filter() {
        ...
        for (subcommand, command) in [
            (Some("next"), "bun next build"),
            (Some("run"), "bun run next build"),
        ] {
    ...
    fn bun_test_failure_keeps_fail_file_and_summary() {
        ...
        assert!(
            out.text.contains("FAIL auth.test.ts"),
            "FAIL line must survive: {:?}",
            out.text
        );
        assert!(
        ...
        assert!(
            out.text.contains("Tests 1 failed"),
            "summary line must survive: {:?}",
            out.text
        );
        assert!(
            out.text.contains("2 passed"),
            "passed count must survive: {:?}",
            out.text
        );
    }
    ...
    fn bun_check_failure_keeps_diagnostic_and_emits_failed_status() {
        ...
        assert!(
            ...
            "failed verdict must appear: {:?}",
            out.text
        );
        assert!(
            out.text.contains("error TS2322"),
