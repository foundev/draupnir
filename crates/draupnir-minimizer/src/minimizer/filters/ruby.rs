...
fn render_rspec_blocks(out: &mut String, blocks: &mut Vec<String>, rendered: &mut bool) {
    ...
    *rendered = true;
    ...
    if total > MAX_RENDERED_FAILURES {
        push_line(
            out,
            &format!("[…{} failures elided…]", total - MAX_RENDERED_FAILURES),
        );
    }
    blocks.clear();
}
...
fn compact_rubocop_autocorrect(line: &str) -> Option<String> {
    ...
    Some(format!(
        "ok rubocop -A ({files} files, {corrected} autocorrected)"
    ))
}
...
mod tests {
    ...
    fn rspec_failure_real_order_keeps_detail_under_failures_header() {
        ...
        // the `Failures:` header.
        assert!(
            failures_at < detail_at,
            "detail must follow Failures header: {out}"
        );
        assert!(detail_at < summary_at, "detail must precede summary: {out}");
        assert!(
            summary_at < failed_examples_at,
            "summary must precede Failed examples list: {out}"
        );
        // The two section headers must not be adjacent (the collision symptom).
        assert!(
            detail_at < failed_examples_at,
            "detail must not land under Failed examples: {out}"
        );
    }
    ...
    fn rake_keep_warning_lines() {
        assert!(is_rake_keep_line(
            "warning: constant Foo::Bar is deprecated"
        ));
