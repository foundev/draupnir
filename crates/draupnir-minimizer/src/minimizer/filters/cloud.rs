                ...
                &rows
                    ...
                    .map(|item| {
                        vec![
                            ...
                            item.get("Endpoint").and_then(Value::as_object).map_or_else(
                                || "-".to_string(),
                                |ep| string_field_map(ep, &["Address"]),
                            ),
                        ]
                    })
                    .collect::<Vec<_>>(),
...
fn compact_aws_eks(root: &Value) -> Option<String> {
    if let Some(values) = root.get("clusters").and_then(Value::as_array) {
        let rows = values
            ...
            .collect::<Vec<_>>();
        return Some(compact_named_rows(
...
mod tests {
    ...
    fn aws_s3_cp_to_stdout_preserves_body() {
        ...
        assert!(
            !out.changed,
            "stdout pipe body must not be rewritten: {:?}",
            out.text
        );
    }
    ...
    fn aws_s3_cp_to_stdout_with_trailing_options_preserves_body() {
        ...
        let ctx = aws_ctx(
            "s3",
            "aws s3 cp s3://bucket/file.json - --request-payer requester",
            &cfg,
        );
        let input = "{\"key\": \"value\", \"% Total\": 100}\n";
        let out = filter(&ctx, input, 0);
    ...
    fn s3_ls_summarize_footer_is_not_parsed_as_row() {
        ...
        let out = filter(&ctx, input, 0);
        assert!(
            out.text.contains("builds\t2026-05-27 01:02:03"),
            "{:?}",
            out.text
        );
