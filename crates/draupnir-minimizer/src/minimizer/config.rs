                ...
                match pipeline::parse_file(&contents, "user") {
                    Ok((pipelines, tests)) => {
                        if !pipelines.is_empty() {
                            cfg.user_pipelines =
                                Some(Arc::new(PipelineRegistry { pipelines, tests }));
                        }
                    }
                    Err(err) => {
...
mod tests {
    ...
    fn resolve_legacy_filters_honors_truthy_env_when_option_absent() {
        for raw in ["1", "true", "yes", " TRUE ", "Yes"] {
            assert!(
                super::resolve_legacy_filters(None, Some(raw)),
                "{raw:?} should enable"
            );
        }
    ...
    fn explicit_disabled_option_overrides_enabled_settings_file() {
        let path = std::env::temp_dir().join(format!(
            "omp-minimizer-config-disabled-{}.toml",
            std::process::id()
        ));
    ...
    fn explicit_legacy_true_overrides_disabled_settings_file() {
        let path = std::env::temp_dir().join(format!(
            "omp-minimizer-config-legacy-{}.toml",
            std::process::id()
        ));
